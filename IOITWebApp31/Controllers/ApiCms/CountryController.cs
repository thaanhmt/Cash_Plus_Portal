    ...
    public class CountryController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            if (paging != null)
            {
                using (var db = new IOITDataContext())
                {
                    ...
                    IQueryable<Country> data = db.Country.Where(c => c.Status != (int)Const.Status.DELETED);
                    ...
                    if (paging.page_size > 0)
                    {
                        ...
                        {
                            data = data.OrderBy("CountryId desc").Skip((paging.page - 1) * paging.page_size).Take(paging.page_size);
                        }
                    }
                    else
                    {
                        ...
                        {
                            data = data.OrderBy("CountryId desc");
                        }
                    }
        ...
        public async Task<IActionResult> GetCountry(int id)
        {
            ...
            try
            {
                using (var db = new IOITDataContext())
                {
                    Country data = await db.Country.FindAsync(id);
        ...
        // PUT: api/Country/5
        [HttpPut("{id}")]
        public async Task<IActionResult> PutCountry(int id, Country data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
                ...
                using (var db = new IOITDataContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (DbUpdateException e)
                        {
                            ...
                            if (!CountryExists(data.CountryId))
                            {
                                def.meta = new Meta(404, "Not Found");
                                return Ok(def);
                            }
            ...
            try
            {
                ...
                using (var db = new IOITDataContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        await db.Country.AddAsync(data);
