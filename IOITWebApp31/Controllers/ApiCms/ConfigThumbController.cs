...
namespace IOITWebApp31.ApiCMS.Controllers
{
    ...
    public class ConfigThumbController : ControllerBase
    {
                    ...
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            if (data.ConfigThumbId > 0)
                            {
                                ...
                                action.ActionName = "Sửa cấu hình thumb “" + data.Name + "”";
                                action.ActionType = (int)Const.ActionType.UPDATE;
                                action.TargetId = data.ConfigThumbId.ToString();
                    ...
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            if (data.ConfigThumbId > 0)
                            {
                                transaction.Commit();
                                Models.EF.Action action = new Models.EF.Action();
                                action.ActionName = "Thêm cấu hình thumb “" + data.Name + "”";
                    ...
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            if (data.ConfigThumbId > 0)
                            {
                                transaction.Commit();
                                Models.EF.Action action = new Models.EF.Action();
                                action.ActionName = "Xoá cấu hình thumb “" + data.Name + "”";
        ...
        //API xóa danh sách config thumb
        [HttpPut("deletes")]
        public async Task<IActionResult> DeleteConfigThumbs([FromBody] int[] data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
            ...
            try
            {
                ...
                using (var db = new IOITDataContext())
                {
                    ...
                    {
                        ConfigThumb configThumb = await db.ConfigThumb.FindAsync(data[i]);
                        ...
                        configThumb.Status = (int)Const.Status.DELETED;
                        db.Entry(configThumb).State = EntityState.Modified;
                        Models.EF.Action action = new Models.EF.Action();
                        action.ActionName = "Xoá cấu hình thumb “" + configThumb.Name + "”";
                        action.ActionType = (int)Const.ActionType.DELETE;
                        action.TargetId = configThumb.ConfigThumbId.ToString();
                        action.TargetName = configThumb.Name;
                        action.CompanyId = companyId;
                        action.Logs = JsonConvert.SerializeObject(configThumb);
                        action.Time = 0;
                        action.Ipaddress = IpAddress();
                        action.Type = (int)Const.TypeAction.ACTION;
                        action.CreatedAt = DateTime.Now;
