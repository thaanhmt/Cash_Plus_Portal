...
namespace IOITWebApp31.Controllers.ApiCms
{
    ...
    {
        private static readonly ILog log = LogMaster.GetLogger("province", "province");
        private static string functionCode = "QLTTP";
            ...
            if (paging != null)
            {
                using (var db = new IOITDataContext())
                {
                    ...
                    if (paging.page_size > 0)
                    {
                        ...
                        {
                            data = data.OrderBy("ProvinceId desc").Skip((paging.page - 1) * paging.page_size).Take(paging.page_size);
                        }
                    }
                    else
                    {
                        ...
                        {
                            data = data.OrderBy("ProvinceId desc");
                        }
                    }
        ...
        public async Task<IActionResult> GetProvince(int id)
        {
            ...
            try
            {
                using (var db = new IOITDataContext())
                {
                    Province data = await db.Province.FindAsync(id);
        ...
        // PUT: api/Province/5
        [HttpPut("{id}")]
        public async Task<IActionResult> PutProvince(int id, Province data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
                ...
                using (var db = new IOITDataContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (DbUpdateException e)
                        {
                            ...
                            if (!ProvinceExists(data.ProvinceId))
                            {
                                def.meta = new Meta(404, "Not Found");
                                return Ok(def);
                            }
        ...
        // POST: api/Province
        [HttpPost]
        public async Task<IActionResult> PostProvince(Province data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
            ...
            try
            {
                ...
                using (var db = new IOITDataContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        await db.Province.AddAsync(data);
