...
namespace IOITWebApp31.Controllers.ApiCms
{
    ...
    public class DistrictController : ControllerBase
    {
        ...
        private static string functionCode = "QLQH";
            ...
            if (paging != null)
            {
                using (var db = new IOITDataContext())
                {
                    ...
                    if (paging.page_size > 0)
                    {
                        ...
                        {
                            data = data.OrderBy("DistrictId desc").Skip((paging.page - 1) * paging.page_size).Take(paging.page_size);
                        }
                    }
                    else
                    {
                        ...
                        {
                            data = data.OrderBy("DistrictId desc");
                        }
                    }
        ...
        public async Task<IActionResult> GetDistrict(int id)
        {
            ...
            try
            {
                using (var db = new IOITDataContext())
                {
                    District data = await db.District.FindAsync(id);
        ...
        // PUT: api/District/5
        [HttpPut("{id}")]
        public async Task<IActionResult> PutDistrict(int id, District data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
                ...
                using (var db = new IOITDataContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (DbUpdateException e)
                        {
                            ...
                            if (!DistrictExists(data.DistrictId))
                            {
                                def.meta = new Meta(404, "Not Found");
                                return Ok(def);
                            }
        ...
        // POST: api/District
        [HttpPost]
        public async Task<IActionResult> PostDistrict(District data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
            ...
            try
            {
                ...
                using (var db = new IOITDataContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        await db.District.AddAsync(data);
