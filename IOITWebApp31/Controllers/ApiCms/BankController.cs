...
namespace IOITWebApp31.ApiCMS.Controllers
{
    ...
    public class BankController : ControllerBase
    {
        ...
        private static string functionCode = "QLNH";
        ...
        public async Task<IActionResult> GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            if (paging != null)
            {
                using (var db = new IOITDataContext())
                {
                    ...
                    IQueryable<Bank> data = db.Bank.Where(c => c.Status != (int)Const.Status.DELETED);
                    ...
                    if (paging.page_size > 0)
                    {
                        ...
                        {
                            data = data.OrderBy("BankId desc").Skip((paging.page - 1) * paging.page_size).Take(paging.page_size);
                        }
                    }
                    else
                    {
        ...
        public async Task<IActionResult> GetBank(int id)
        {
            ...
            try
            {
                using (var db = new IOITDataContext())
                {
                    Bank data = await db.Bank.FindAsync(id);
        ...
        // PUT: api/Bank/5
        [HttpPut("{id}")]
        public async Task<IActionResult> PutBank(int id, Bank data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
        ...
        // POST: api/Bank
        [HttpPost]
        public async Task<IActionResult> PostBank(Bank data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
