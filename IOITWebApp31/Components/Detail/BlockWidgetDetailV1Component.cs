...
{
    [ViewComponent(Name = "BlockWidgetDetailV1")]
    public class BlockWidgetDetailV1Component : ViewComponent
    {
        public BlockWidgetDetailV1Component()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    var DBFirstChilren = db.Category.Where(tc => tc.CategoryParentId == CategoryChildrenId && tc.Status != (int)Const.Status.DELETED).FirstOrDefault();
                    if (DBFirstChilren != null)
                    {
                        ...
                        return await Task.FromResult((IViewComponentResult)View("BlockWidgetDetailV1", data));
                    }
                    else
                    {
                        ViewBag.FirstNameCateWidget = "Chính trị xã hội";
                        var FirstChilrenId = 4563;
                        IEnumerable<News> data = (from cm in db.CategoryMapping
                                                  join n in db.News on cm.TargetId equals n.NewsId
                                                  where cm.TargetType == (int)Const.TypeCategoryMapping.CATEGORY_NEWS
                                                  && n.TypeNewsId == (int)Const.TypeNews.NEWS_TEXT
