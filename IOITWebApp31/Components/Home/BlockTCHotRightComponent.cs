...
{
    [ViewComponent(Name = "BlockTCHotRight")]
    public class BlockTCHotRightComponent : ViewComponent
    {
        public BlockTCHotRightComponent()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                IEnumerable<News> data = (from cm in db.CategoryMapping
                                          ...
                                          select n).OrderByDescending(e => e.DateStartActive).Skip(3).Take(Number).ToList();

                return await Task.FromResult((IViewComponentResult)View("BlockTCHotRight", data));
            }
        }
