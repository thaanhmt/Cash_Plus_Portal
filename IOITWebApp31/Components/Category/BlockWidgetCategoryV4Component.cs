...
{
    [ViewComponent(Name = "BlockWidgetCategoryV4")]
    public class BlockWidgetCategoryV4Component : ViewComponent
    {
        public BlockWidgetCategoryV4Component()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    var DBFirstChilren = db.Category.Where(tc => tc.CategoryParentId == CategoryChildrenId && tc.CategoryId != 4647 && tc.Status != (int)Const.Status.DELETED).Skip(3).FirstOrDefault();
                    if (DBFirstChilren != null)
                    {
                        ...
                        return await Task.FromResult((IViewComponentResult)View("BlockWidgetCategoryV4", data));
                    }
                    else
                    {
