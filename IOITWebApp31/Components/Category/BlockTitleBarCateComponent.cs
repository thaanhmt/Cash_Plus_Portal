...
{
    [ViewComponent(Name = "BlockTitleBarCate")]
    public class BlockTitleBarCateComponent : ViewComponent
    {
        public BlockTitleBarCateComponent()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, Boolean IsDanger, Boolean IsCarouse)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    if (IsDanger == true)
                    {
                        ViewBag.Class = "category-title-bar";
                    }
                    if (IsCarouse == true)
                    {
                        ViewBag.Carouse = "carouseTitleBar";
                    }
                    return await Task.FromResult((IViewComponentResult)View("BlockTitleBarCate", data));
                }
                else
                {
                    IEnumerable<Category> data = (from c in db.Category
