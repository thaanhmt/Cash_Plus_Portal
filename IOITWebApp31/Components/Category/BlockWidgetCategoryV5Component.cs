...
{
    [ViewComponent(Name = "BlockWidgetCategoryV5")]
    public class BlockWidgetCategoryV5Component : ViewComponent
    {
        public BlockWidgetCategoryV5Component()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    var DBFirstChilren = db.Category.Where(tc => tc.CategoryParentId == CategoryChildrenId && tc.CategoryId != 4647 && tc.Status != (int)Const.Status.DELETED).Skip(4).FirstOrDefault();
                    if (DBFirstChilren != null)
                    {
                        ...
                        return await Task.FromResult((IViewComponentResult)View("BlockWidgetCategoryV5", data));
                    }
                    else
                    {
