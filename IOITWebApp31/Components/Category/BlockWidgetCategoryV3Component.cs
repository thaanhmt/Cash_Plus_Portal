...
{
    [ViewComponent(Name = "BlockWidgetCategoryV3")]
    public class BlockWidgetCategoryV3Component : ViewComponent
    {
        public BlockWidgetCategoryV3Component()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    var DBFirstChilren = db.Category.Where(tc => tc.CategoryParentId == CategoryChildrenId && tc.CategoryId != 4647 && tc.Status != (int)Const.Status.DELETED).Skip(2).FirstOrDefault();
                    if (DBFirstChilren != null)
                    {
                        ...
                        return await Task.FromResult((IViewComponentResult)View("BlockWidgetCategoryV3", data));
                    }
                    else
                    {
