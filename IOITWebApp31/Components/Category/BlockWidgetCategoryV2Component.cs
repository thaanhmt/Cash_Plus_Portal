...
{
    [ViewComponent(Name = "BlockWidgetCategoryV2")]
    public class BlockWidgetCategoryV2Component : ViewComponent
    {
        public BlockWidgetCategoryV2Component()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    var DBFirstChilren = db.Category.Where(tc => tc.CategoryParentId == CategoryChildrenId && tc.CategoryId != 4647 && tc.Status != (int)Const.Status.DELETED).Skip(1).FirstOrDefault();
                    if (DBFirstChilren != null)
                    {
                        ...
                        return await Task.FromResult((IViewComponentResult)View("BlockWidgetCategoryV2", data));
                    }
                    else
                    {
