...
{
    [ViewComponent(Name = "BlockChildrenVideoCate")]
    public class BlockChildrenVideoCateComponent : ViewComponent
    {
        public BlockChildrenVideoCateComponent()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                return await Task.FromResult((IViewComponentResult)View("BlockChildrenVideoCate", data));
            }
        }
