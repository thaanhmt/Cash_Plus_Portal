...
{
    [ViewComponent(Name = "BlockNewsBottomCategory")]
    public class BlockNewsBottomCategoryComponent : ViewComponent
    {
        public BlockNewsBottomCategoryComponent()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int CategoryId, int Number)
        {
            using (var db = new IOITDataContext())
            {
                ...
                ViewBag.NameCategoryBottom = db.Category.Find(CategoryId).Name;
                return await Task.FromResult((IViewComponentResult)View("BlockNewsBottomCategory", data));
                ...
                //                              && cm.Status != (int)Const.Status.DELETED
                //                              select n).OrderByDescending(e => e.DateStartActive).Skip(1).Take(Number).ToList();
                //    ViewBag.NameCategoryBottom = db.Category.Find(ParentId).Name;
                //    return await Task.FromResult((IViewComponentResult)View("BlockNewsBottomCategory", data));
                //}               
            }
        }
