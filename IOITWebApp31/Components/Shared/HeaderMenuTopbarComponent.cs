...
{
    [ViewComponent(Name = "HeaderMenuTopbar")]
    public class HeaderMenuTopbarComponent : ViewComponent
    {
        public HeaderMenuTopbarComponent()
        {
        }
        public async Task<IViewComponentResult> InvokeAsync(int MenuId, int MenuParentId, List<MenuItems> menuItems)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (MenuParentId == 0)
                {
                    ...
                    return await Task.FromResult((IViewComponentResult)View("HeaderMenuTopbar", data));
                }
                else
                {
                    return await Task.FromResult((IViewComponentResult)View("HeaderMenuTopbar", menuItems));
                }
            }
        }
    }
