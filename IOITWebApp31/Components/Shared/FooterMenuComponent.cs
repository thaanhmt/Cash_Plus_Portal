        ...
        public async Task<IViewComponentResult> InvokeAsync(int MenuId, int MenuParentId, List<MenuItems> menuItems)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (menuItems != null)
                {
                    ...
                    return await Task.FromResult((IViewComponentResult)View("FooterMenu", data));
                }
                else
                {
                    return await Task.FromResult((IViewComponentResult)View("FooterMenu", menuItems));
                }
            }
        }
    }
