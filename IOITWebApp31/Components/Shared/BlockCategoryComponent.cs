...
{
    [ViewComponent(Name = "BlockCategory")]
    public class BlockCategoryComponent : ViewComponent
    {
        public BlockCategoryComponent()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int MenuId, int MenuParentId, List<MenuItems> menuItems)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (MenuParentId == 0)
                {
                    ...
                    return await Task.FromResult((IViewComponentResult)View("BlockCategory", data));
                }
                else
                {
                    return await Task.FromResult((IViewComponentResult)View("BlockCategory", menuItems));
                }
            }
        }
    }
