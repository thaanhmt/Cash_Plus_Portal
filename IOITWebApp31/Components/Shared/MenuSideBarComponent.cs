...
namespace IOITWebApp31.Components.Shared
{
    ...
    public class MenuSideBarComponent : ViewComponent
    {
        public MenuSideBarComponent()
        {
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync(int MenuId, int MenuParentId, List<MenuItems> menuItems)
        {
            using (var db = new IOITDataContext())
            {
                ...
                if (MenuParentId == 0)
                {
                    ...
                    return await Task.FromResult((IViewComponentResult)View("MenuSideBar", data));
                }
                else
                {
                    return await Task.FromResult((IViewComponentResult)View("MenuSideBar", menuItems));
                }
            }
        }
    }
