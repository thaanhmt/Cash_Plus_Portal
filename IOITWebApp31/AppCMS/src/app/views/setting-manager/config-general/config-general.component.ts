...
import { Config, Website } from '../../../data/model';
import { ToastrService } from 'ngx-toastr';
import { domainDebug, domainMedia, TypeUpload } from '../../../data/const';
...
export class ConfigGeneralComponent implements OnInit {
  ...
  GetInfoWebsite() {
    this.http.get('api/website/1', this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.Item = new Website();
          this.Item = JSON.parse(JSON.stringify(res["data"]));
