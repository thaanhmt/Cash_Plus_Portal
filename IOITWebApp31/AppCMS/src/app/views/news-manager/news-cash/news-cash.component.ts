...
import { typeCategoryNews, domainImage, ActionTable, NewsStatus, domain, listCashNews, domainMedia, domainDebug } from '../../../data/const';
...
export class NewsCashComponent implements OnInit {
  ...
  public listCashNews = listCashNews;
  ...
  GetListAllNews() {
    ...
    this.http.get('/api/news/GetByPage?page=1&page_size=100&query=' + query, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
  ...
  DeleteNews(Id) {
    if (this.RoleCode == 'ADMIN') {
      if (this.activeU == true) {
        this.http.delete('/api/news/DeleteNewsPublic/' + Id, this.httpOptions).subscribe(
          (res) => {
            if (res["meta"]["error_code"] == 200) {
              this.GetListNews();
              this.viewRef.clear();
    ...
    this.http.request(uploadReq).subscribe(event => {
      if (event.type === HttpEventType.UploadProgress)
        ...
        switch (cs) {
          case 1:
            this.message = event.body["data"].toString();
            this.Item.LinkVideo = this.message;
            break;
          case 2:
            this.attachment.nativeElement.value = "";
                    /*console.log(event.body["data"])*/;
  ...
  ShowConfirmStatus(id, status) {
    let title = "Bạn có chắc chắn muốn cập nhật nhuận bút bài viết này?";
    //if (status == 19)
  ...
  SendStatus(id, status) {
    this.http.put('/api/news/ShowHide/' + id + "/" + status + "/0", undefined, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          if (status == 1)
            this.toastSuccess("Bài viết đã được cập nhật thành công!");
          //else
  ...
  ExportExcel() {
    ...
    this.http.post<Blob>('/api/news/exportNewsCash', data,this.httpOptionsBlob).subscribe(
      (res) => {
        //console.log(res);
