...
export class NewsBrowsingComponent implements OnInit {
  ...
  GetListAllNews() {
    ...
    //     }
    // }
    this.http.get('/api/news/GetByPage/?page=1&query=' + query + '&order_by=&select=NewsId,Title,Url,Image,TypeNewsId,LanguageId', this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
  ...
  //
  QueryChanged() {
    let query = "1=1 AND (Status=16 || Status=17)";
      ...
      this.http.post('/api/news', obj, this.httpOptions).subscribe(
        (res) => {
          if (res["meta"]["error_code"] == 200) {
            ...
            this.toastError(res["meta"]["error_code"] == 228);
          }
        },
        (err) => {
          this.toastError("Đã xảy ra lỗi. Xin vui lòng thử lại sau!");
  ...
  DeleteNews(Id) {
    this.http.delete('/api/news/' + Id + '/3', this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.GetListNews();
          this.viewRef.clear();
  ...
  ConfirmShowHideBrowsing(item, i) {
    if (item.listLanguage.length > 0) {
      this.modalDialogService.openDialog(this.viewRef, {
        title: 'Xác nhận',
        childComponent: SimpleModalComponent,
        ...
        actionButtons: [
          {
            ...
            buttonClass: 'btn btn-success',
            onAction: () => {
              this.ShowHideBrowSing(item.NewsId, i, 1);
            }
          },
          {
            text: 'Không đồng ý',
  ...
  ShowHideBrowSing(id, i, isAll) {
    if (this.CheckConfirmNews == true) {
      let stt = this.listNews[i].IsShow ? 12 : 11;
        ...
        (res) => {
          if (res["meta"]["error_code"] == 200) {
            if (stt == 12) {
              ...
              this.toastWarning("Bài viết đã chuyển về cho tác giả");
            }
            this.GetListNews();
            this.viewRef.clear();
          }
        ...
        actionButtons: [
          {
            ...
            buttonClass: 'btn btn-success',
            onAction: () => {
              this.ShowHidePublic(item.NewsId, i, 1);
            }
          },
          {
            text: 'Không đồng ý',
  ...
  ShowHidePublic(id, i, isAll) {
    if (this.CheckConfirmNews == true) {
      let sttPublic = this.listNews[i].IsPublic ? 1 : 12;

      this.http.put('/api/news/ShowHide/' + id + "/" + sttPublic + "/" + isAll, undefined, this.httpOptions).subscribe(
        (res) => {
          if (res["meta"]["error_code"] == 200) {
            ...
            this.toastError("Đã xảy ra lỗi. Xin vui lòng thử lại sau!");
            this.listNews[i].IsPublic = !this.listNews[i].IsPublic;
          }
  ...
  ShowConfirmStatus(id, status) {
    let title = "Bạn có chắc chắn duyệt bài viết này?";
    if (status == 17)
  ...
  SendStatus(id, status) {
    this.http.put('/api/news/ShowHide/' + id + "/" + status + "/0", undefined, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          if (status == 18)
            this.toastSuccess("Bài viết đã được duyệt thành công!");
          else if (status == 17)
            this.toastSuccess("Bài viết đã không được duyệt thành công!");
          else if (status == 15)
            this.toastSuccess("Bài viết đã được trả biên tập lại thành công!");
          this.GetListNews();
          this.NewsModal.hide();
          this.viewRef.clear();
        }
