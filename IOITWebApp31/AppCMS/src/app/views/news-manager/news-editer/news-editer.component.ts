...
import { typeCategoryNews, domainImage, ActionTable, Status, domain, listHotNews, domainVideos, domainMedia, domainDebug } from '../../../data/const';
...
export class NewsEditerComponent implements OnInit {
  ...
  constructor(
    ...
    //
    this.CheckRoleAp = new CheckRole();
    let codeAp = "NB";
  ...
  //
  QueryChanged() {
    let query = "1=1 AND (Status=13 OR Status=14 OR Status=15)";
  ...
  DeleteNews(Id) {
    ...
    //  if (this.activeU == true) {
        this.http.delete('/api/news/DeleteNewsPublic/' + Id + '/2', this.httpOptions).subscribe(
          (res) => {
            if (res["meta"]["error_code"] == 200) {
              this.GetListNews();
              this.viewRef.clear();
      ...
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          if (stt == 1) {
            ...
            this.toastSuccess("Bài viết đã được gửi kiểm duyệt!");
          } else {
  ...
  ShowConfirmStatus(id, status) {
    let title = "Bạn có chắc chắn muốn gửi kiểm duyệt bài viết này?";
    if (status == 12)
  ...
  SendStatus(id, status) {
    this.http.put('/api/news/ShowHide/' + id + "/" + status + "/0", undefined, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          if(status != 12)
            this.toastSuccess("Bài viết đã được gửi kiểm duyệt thành công!");
          else
            this.toastSuccess("Bài viết đã được gửi trả lại cho PV/CTV thành công!");
          this.GetListNews();
          this.viewRef.clear();
        }
        else {
